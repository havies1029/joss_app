...
class HakaksesCrudBloc extends Bloc<HakaksesCrudEvents, HakaksesCrudState> {
	...
	HakaksesCrudBloc({required this.repository}) : super(const HakaksesCrudState()) {
		on<HakaksesCrudLihatEvent>(onLihatHakaksesCrud);
	}

	Future<void> onLihatHakaksesCrud(
		HakaksesCrudLihatEvent event, Emitter<HakaksesCrudState> emit) async {
		emit(state.copyWith(isLoading: true, isLoaded: false));
    
		HakaksesCrudModel record = await repository.hakaksesCrudLihat();
