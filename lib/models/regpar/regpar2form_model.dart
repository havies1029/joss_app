...
class Regpar2FormModel {
  ...
  factory Regpar2FormModel.fromJson(Map<String, dynamic> data) {
    ...
    } else if ((data['rkonstruksiojkId'] ?? '').toString().trim().isNotEmpty ||
        (data['kelasNama'] ?? '').toString().trim().isNotEmpty) {
      comboRKonstruksiojk = ComboRKonstruksiojkModel(
        rkonstruksiojkId: data['rkonstruksiojkId']?.toString() ?? '',
        kelasNama: data['kelasNama']?.toString() ?? '',
      );
    }
    ...
    } else if ((data['rokupasiId'] ?? '').toString().trim().isNotEmpty ||
        (data['okupasiDesc'] ?? '').toString().trim().isNotEmpty) {
      comboROkupasi = ComboROkupasiModel(
        rokupasiId: data['rokupasiId']?.toString() ?? '',
        okupasiDesc: data['okupasiDesc']?.toString() ?? '',
      );
    }
